  ...
  return (
    <div className="space-y-3 md:space-y-8">
      ...
      <div className="text-center">
        <p className="text-lg text-gray-700 mb-2 md:mb-4">{question_data.prompt}</p>

        {/* Analogy display */}
        {question_data.type === 'analogy' && question_data.sequence && (
          <div className="inline-flex items-center gap-2 md:gap-3 p-3 md:p-6 bg-gray-100 rounded-xl text-base md:text-xl">
      ...
      {/* Answer options */}
      <div>
        <p className="text-sm text-gray-500 mb-2 md:mb-4 text-center">Select the best answer:</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-4 max-w-2xl mx-auto">
          {options.map((option) => (
            <button
              key={option.id}
              onClick={() => onSelectAnswer(option.id)}
              className={`p-2.5 md:p-4 rounded-xl border-2 text-left transition-all ${
            ...
            >
              <div className="flex items-center gap-2 md:gap-4">
                <span className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                  selectedAnswer === option.id
                ...
                <span className="text-base md:text-lg text-gray-900 font-medium">{option.value}</span>
              </div>
            </button>
          ))}
        </div>
